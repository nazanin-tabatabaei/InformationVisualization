...
window.onload = start;
var color_used="steelblue";
function onCategoryChanged() {
    var select = d3.select('#categorySelect').node();
    // Get current value of select element
    color_used = select.options[select.selectedIndex].value;
}

//var vals=d3.select('#cutoff_amnt').node();
//ar cut=vals.value;
...
function start() {
    ...
    d3.select(graph)
        ...
        .on('click', function() {
            ...
            cut=parseFloat(document.getElementById("cutoff_amnt").value);
            updateChart(1400,color_used,cut);
    ...
    updateChart(0,"steelblue",0);
    });




    function updateChart(deley1,filterKey,cutff){
    //bars.selectAll('.bar').remove();
